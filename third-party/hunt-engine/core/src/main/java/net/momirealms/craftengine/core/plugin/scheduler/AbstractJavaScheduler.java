    ...
    public void shutdownScheduler() {
        ...
        try {
            if (!this.scheduler.awaitTermination(1, TimeUnit.MINUTES)) {
                this.plugin.logger().error("Timed out waiting for the HuntEngine scheduler to terminate");
    ...
    public void shutdownExecutor() {
        ...
        try {
            if (!this.worker.awaitTermination(1, TimeUnit.MINUTES)) {
                this.plugin.logger().error("Timed out waiting for the HuntEngine worker thread pool to terminate");
