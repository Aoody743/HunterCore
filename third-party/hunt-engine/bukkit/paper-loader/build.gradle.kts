...
dependencies {
    ...
    // Keep every runtime library inside HuntEngine.jar. The vendor source
    // intentionally disables the upstream startup downloader.
...
tasks {
    shadowJar {
        ...
        // Keep the upstream nested jar for dev-time proxy diagnostics, but
        // also embed its classes in the main offline distribution. The
        // HunterCore dependency manager intentionally never downloads or
        // extracts jars at first startup, while BukkitCraftEngine needs
        // BukkitProxy during bootstrap.
        ...
        from(proxyShadowJar.flatMap { it.archiveFile }.map { zipTree(it.asFile) }) {
            // The proxy jar also contains a relocated sparrow-reflection
            // copy.  HuntEngine shades that library itself, so importing the
            // entire proxy jar produces duplicate classes/resources whose
            // winner depends on archive input order.  Only the bootstrap
            // proxy classes are needed in the outer, offline jar; the intact
            // proxy.jarinjar remains above for development diagnostics.
        ...
        archiveFileName = "HuntEngine.jar"
