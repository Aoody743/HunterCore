...
public final class BukkitSenderFactory extends SenderFactory<BukkitCraftEngine, CommandSender> {
    ...
    protected Tristate permissionState(CommandSender sender, String node) {
        if (sender.hasPermission(node)) {
            ...
            final String legacyNode = legacyHunterAssetsPermission(node);
            if (legacyNode == null) {
                return Tristate.UNDEFINED;
            }
            if (sender.hasPermission(legacyNode)) {
                return Tristate.TRUE;
            }
            return sender.isPermissionSet(legacyNode) ? Tristate.FALSE : Tristate.UNDEFINED;
        }
    }
    ...
    protected boolean hasPermission(CommandSender sender, String node) {
        ...
        final String legacyNode = legacyHunterAssetsPermission(node);
        return legacyNode != null && sender.hasPermission(legacyNode);
    }

    /**
     * HunterAssets permission aliases remain intentionally narrow through the 2.9.x line.
     *
     * <p>They are resolved only when an administrator has not explicitly set the corresponding
     * HuntEngine node. This lets a server migrate its permission rules incrementally while an
     * explicit new deny always wins over a legacy grant. The aliases are removed with the 2.10.0
     * compatibility layer; upstream CraftEngine permissions are never exposed here.</p>
     */
    private static String legacyHunterAssetsPermission(String node) {
        return switch (node) {
            case "huntengine.use" -> "hunterassets.use";
            case "huntengine.admin" -> "hunterassets.admin";
            case "huntengine.items.give" -> "hunterassets.give";
            case "huntengine.item.*" -> "hunterassets.item.*";
            default -> node.startsWith("huntengine.item.")
                ? "hunterassets.item." + node.substring("huntengine.item.".length())
                : null;
        };
    }
