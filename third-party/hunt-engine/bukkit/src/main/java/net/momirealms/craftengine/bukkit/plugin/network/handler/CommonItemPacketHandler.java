            ...
            if (!ItemStackProxy.CLASS.isInstance(nmsItemStack)) {
                ...
                if (time - lastWarningTime > 5000) {
                    ...
                    CraftEngine.instance().logger().error("An issue was detected while applying item-related entity data for '" + serverPlayer.name() +
                            "'. Please execute the command '/huntengine debug entity-id " + serverPlayer.world().name() + " " + id + "' and provide a screenshot for further investigation. Class: " + nmsItemStack.getClass() + ". Object: " + GsonHelper.get().toJson(nmsItemStack));
